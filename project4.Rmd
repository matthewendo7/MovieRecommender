...
Key Paramters for IBCF

- Normlization method(normalize):- Z-score. Normalization method to remove the rating bias of rating matrices
- Distance Method(method): Cosine Similarity
- k = 20. Number of similar products to identify(Number of latent attributes of dimension)

Key Parameters for UBCF

- Normlization method(normalize):- Z-score. Normalization method to remove the rating bias of rating matrices
- Distance Method(method): Cosine Similarity
- nn = 25, nn is the number of nearest neighbors
- Weight = TRUE. By setting this parameter to TRUE, the average ratings are weighted according to their similarity
...
```{r}
RMSE_summary = data.frame(Iteration = seq(1,10), IBCF_RMSE = ibcf_rmse, UBCF_RMSE = ubcf_rmse)
RMSE_summary
```
