...
top_n_most_popular <- function(movie_rating_df, genre, n = 5){
  ...
  s$Genre = genre
  return(s[order(-s$rating_count),][1:n,c("Title","Genre","rating_count")])
}
```
...
The system we implemented uses UBCF.  UBCF answers the question of whether a user will like a specific movie or not by comparing the current user to other users in the system, and evaluating what those users prefer.  In this case, we have the reviews for thousands of users. When the current users rates movies, the similarity of the current user to other users in the dataset can be measured. The recommender system can then recommend the top rated movies of similar users.

### Approach 2

An alternative approach is IBCF.  
